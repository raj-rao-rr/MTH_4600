...
int main () {
   ...
   double s_start, s, T, r, sigma, sigma2, sigma2_start, mu, alpha, beta,
          gamma, dt, N01, R, U, V2, K, vol, payoff, avg1, discount_factor;

   // strikes for XOM options
   ...
   // average option payoff per each strike
   ...
   // discount factor for the price of cashflows
   discount_factor = exp(-0.05 * 0.5);
   ...
   for (n = 1; n <= sims; ++n) {
      ...
      // Problem 1 //
      avg1 = (avg1 * (n - 1) + s * discount_factor) / n; // since K = 0, we simply find the average of terminal stock price 

      // Problem 2 //
      for (j = 0; j < 11; ++j) {
          // itterate through each strike value and subtract from terminal strike price  
          payoff = s - strikes[j];
          averages[j] = (averages[j] * (n - 1) + Max(payoff) * discount_factor) / n;
      }
   ...
   for (int t = 0; t < 11; ++t) {
       K = strikes[t];
       vol = 100.0*ImpliedVol(0.5, 100, K, 0.05, averages[t]);
