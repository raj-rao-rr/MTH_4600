...
void   MakeParCurve (double *);
void   ComputeCurves (double *, double *, double *, double *);
double Hump (double);
double Twist (double);
...
// for -5 <= x <= 5.  For values of x outside this
...
////////////////////////////////////////////////////////////////////////////////
// This function creates a par yield curve.
// The curve is reported to the array "par[]".
////////////////////////////////////////////////////////////////////////////////
void MakeParCurve (double *par) {

   int n;
   double rate, A, B, C, t, shortrate, longrate, hump;
   FILE *fp;
   ...
   //   A + B * Twist (30.0) + C * Hump (30.0) = "longrate",
   //   when C = "hump".
   C = hump;
   ...
   for (n = 1; n <= 60; n++) {
      ...
      rate = A + B * Twist(t) + C * Hump(t);

      // Report back par[n] as a decimal.
      par[n] = rate / 100.0;
   ...
   fprintf (fp, "%6.2f %8.5f  %6.2f %8.5f  %6.2f %8.5f\n",
                0.5, shortrate, 4.0, 100.0*par[8], 30.0, longrate);
   fclose (fp);
...
////////////////////////////////////////////////////////////////////////////////
// From the par curves this function computes the other three.
// Each is held as a decimal with semiannual compounding.
////////////////////////////////////////////////////////////////////////////////
void ComputeCurves (double *par, double *discount, double *zero,
                    double *forward) {
   ...
   for (n = 1; n <= 60; n++) {
      ...
      discount[n] = (1.0 - coupons) / (1.0 + par[n] * 0.5);
   }

   // Step 2: now that the discount function d[] is computed...

   // ... compute the zero coupon curve,
   for (n = 1; n <= 60; n++) {
      zero[n] = 2.0 * (pow(discount[n], -1.0 / n) - 1.0);
   }

   // ... and finally compute single-period forward rates.
   for (n = 0; n < 60; n++) {
      forward[n] = 2.0 * (pow (discount[n+1]/discount[n], -1) - 1.0);
   }

   // Report the three curves to output files if so instructed. Report them as
   //   percents rather than decimals.
